...
async function startServer() {
  ...
  // This ensures that when the user inputs or updates their secret "OPEN_API" key,
  // it is immediately picked up by the app without waiting for a server restart.
  ...
  function checkIsQuotaError(error: any): boolean {
    ...
    // Convert properties to dynamic string references because standard error properties (like .message)
    // are not serializable by JSON.stringify() by default.
    const pieces = [
      String(error),
  ...
  // 1. Analyze Wins Endpoint (Highly Optimized Prompt & Data Slay to Save Tokens)
  ...
  // 2. Chat with AI Endpoint (Database Research Engine & Multi-Win Contextualizer)
  app.post("/api/chat-with-ai", async (req, res) => {
    const { message, history, wins, userName } = req.body;

    // A. Generate embedding for the query message to perform semantic similarity matching
    ...
    // B. Tokenize and filter terms for exact/keyword matching
    ...
    // C. Perform multi-layer relevance analysis across all items in the database
    let totalCount = 0;
    let habitCount = 0;
    let beDoHaveCount = 0;
      ...
      for (const w of wins) {
        ...
        for (const word of queryWords) {
          ...
          if (tagsLower.includes(word)) {
            score += 20; // high boost for exact tag topic matching
            keywordMatchCount++;
          }
        }
        ...
        // 4. Recency boost (newer wins are favored slightly)
        if (w.createdAt) {
          const t = typeof w.createdAt === 'number' ? w.createdAt : new Date(w.createdAt).getTime();
    ...
    // Sort wins by matching relevance score descending
    scoredWins.sort((a, b) => b.score - a.score);

    // Format top 20 most relevant analyzed wins
    const selectedWins = scoredWins.slice(0, 20).map(item => item.win);
    
    // Sort selected wins chronologically so the LLM receives a coherent timeline
...
Desain jawaban kamu berdasarkan hasil penelitian di atas. Rujuk riwayat kemenangan spesifik mereka (sebut tgl, topik, isi kemenangan, refleksi mereka) untuk memvalidasi kemajuan mereka dan merumuskan saran konkrit baru. Tunjukan bahwa kamu benar-benar meneliti sejarah sukses mereka dalam menjawab!`;
    ...
    try {
      // KEEP ONLY the last 6 messages to save massive amounts of input tokens (holds 3 full roundtrip conversations)
